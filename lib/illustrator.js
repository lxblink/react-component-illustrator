  ...
  _createClass(Illustrator, [{
    ...
    value: function processComponent(file) {
      ...
      return _Promise.resolve(file).then(this.record('componentPath')).then(function (file) {
        ...
        return _fs2['default'].readFileSync(file, { encoding: 'utf-8' });
      }).then(this.record('componentSource')).then(function (source) {
        if ((0, _util.isTs)(file)) {
          return _this3.parseComponentDocTs(source);
        } else {
          return _this3.parseComponentDoc(source);
        }
    ...
    key: 'parseComponentDocTs',
    value: function parseComponentDocTs(code) {
      return (0, _reactDocgenTypescript.parse)(code);
    }
  }, {
